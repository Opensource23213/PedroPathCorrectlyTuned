 ...
 */
@Config
@Autonomous (name = "RedBucketAuto", group = "AAA", preselectTeleOp = "Bucket_Tele")
public class RedBucket extends OpMode {
    ...
    public void init() {
        ...
        push1ish = new Path(new BezierLine(new Point(13.2, 9.7, Point.CARTESIAN), new Point(20.5, 9.5, Point.CARTESIAN)));
            ...
            if(stopped == 2){
                if(forward < 7) {
                    if (forward == 1) {
                        ...
                        drivetime.reset();
                        forward = 5.25;
                    } else if(forward == 5.25 && drivetime.time(TimeUnit.MILLISECONDS) > 700){
                        gripspinny.setPower(1);
                        drivetime.reset();
                        forward = 5.5;
                    }else if(forward == 5.5 && drivetime.time(TimeUnit.MILLISECONDS) > 200){
                        wristpose = .293;
                        flippose = .613;
                        if(abs(wristpose - wrist_at) < .05) {
                            armtarget = 0;
                    ...
                    if (forward == 7) {
                        ...
                        drivetime.reset();
                        forward = 9.25;
                    } else if(forward == 9.25 && drivetime.time(TimeUnit.MILLISECONDS) > 600){
                        gripspinny.setPower(1);
                        drivetime.reset();
                        forward = 9.5;
                    }else if(forward == 9.5 && drivetime.time(TimeUnit.MILLISECONDS) > 400){
                        wristpose = .293;
                        flippose = .613;
                        if(abs(wristpose - wrist_at) < .05) {
                            armtarget = 0;
                        ...
                        drivetime.reset();
                        forward = 13.25;
                    } else if(forward == 13.25 && drivetime.time(TimeUnit.MILLISECONDS) > 600){
                        gripspinny.setPower(1);
                        drivetime.reset();
                        forward = 13.5;
                    } else if(forward == 13.5 && drivetime.time(TimeUnit.MILLISECONDS) > 400){
                        wristpose = .293;
                        flippose = .613;
                        if(abs(wristpose - wrist_at) < .05) {
                            armtarget = 1890;
